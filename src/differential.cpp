...
void AlleleTestLauncher::perform_testing(vector<shared_ptr<SampleAlleleAbundances> >& abundances)
{
	...
	const SampleAlleleAbundances& curr = *(abundances[0]);
	const AlleleAbundanceGroup& s1 = curr.transcripts;
	test_differential(abundances.front()->locus_tag, abundances, _contrasts, *_tests, *_tracking);
}
                              ...
                              const FPKMContext& prev)
{
    ...
    if ((curr.FPKM != 0 || prev.FPKM != 0) && (prev.fpkm_samples.size() > 0 && curr.fpkm_samples.size() > 0))
    {
        ...
        //Note to cole: this is not the traditional permutation test for assessing the significance of mean(A)-mean(B), where a null is created by mixing A and B and creating a distribution of mean(A')-mean(B') where A' and B' are samples from A,B. The test implemented here may inflate p-values.
		//allele
		double min_non_zero_prev_sample,min_non_zero_curr_sample;
		vector<double> sorted_prev_samples = prev.fpkm_samples;
		vector<double> sorted_curr_samples = curr.fpkm_samples;
		vector<double>::iterator sItr;
		sort(sorted_prev_samples.begin(),sorted_prev_samples.end());
		sort(sorted_curr_samples.begin(),sorted_curr_samples.end());
		sItr = sorted_prev_samples.begin();
		while(sItr != sorted_prev_samples.end())
		{
			if(*sItr > 0)
			{
				min_non_zero_prev_sample = *sItr;
				break;
			}
			sItr++;
		}
		sItr = sorted_curr_samples.begin();
		while(sItr != sorted_curr_samples.end())
		{
			if(*sItr > 0)
			{
				min_non_zero_curr_sample = *sItr;
				break;
			}
			sItr++;
		}
        ...
        for (size_t i = 0; i < num_null_ratio_samples; ++i)
        {
            ...
            //allele
			if(curr_set_sample == 0) curr_set_sample = min_non_zero_prev_sample;
			if(prev_set_sample == 0) prev_set_sample = min_non_zero_prev_sample;
            double null_ratio_sample = log2(curr_set_sample) - log2(prev_set_sample);
			
			/*This will create problems if the mass of the fpkm samples is close to 0
        ...
        for (size_t i = 0; i < num_null_ratio_samples; ++i)
        {
            ...
            //allele
			if(curr_set_sample == 0) curr_set_sample = min_non_zero_curr_sample;
			if(prev_set_sample == 0) prev_set_sample = min_non_zero_curr_sample;
			double null_ratio_sample = log2(curr_set_sample) - log2(prev_set_sample);
			/*This will create problems if the mass of the fpkm samples is close to 0
            double null_ratio_sample = 0.0;
            if (curr_set_sample > 0.0 && prev_set_sample > 0.0)
							  ...
							  string desc)
{
    ...
    vector<double> null_log_ratio_samples,all_samples;
    
    static const size_t num_null_ratio_samples = 20000;
    
    boost::random::mt19937 rng;
    
    if ((curr.FPKM != 0 || prev.FPKM != 0) && (prev.fpkm_samples.size() > 0 && curr.fpkm_samples.size() > 0))
    {
		boost::random::uniform_int_distribution<> all_sampler(0, curr.fpkm_samples.size()+prev.fpkm_samples.size()-1);
		...
		//shuffle all_samples only once
		random_shuffle ( all_samples.begin(), all_samples.end() );
		// Draw from null
        for (size_t i = 0; i < num_null_ratio_samples; ++i)
        {
			double curr_set_sample = 0.0;
            for (size_t k = 0; k < curr.tracking_info_per_rep.size(); ++k)
            {
                int next_sample_idx = all_sampler(rng);
                if (next_sample_idx >= 0 && next_sample_idx < all_samples.size())
                    curr_set_sample += all_samples[next_sample_idx] / (double)curr.tracking_info_per_rep.size();
            }
            
            double prev_set_sample = 0.0;
            for (size_t k = 0; k < prev.tracking_info_per_rep.size(); ++k)
            {
                int next_sample_idx = all_sampler(rng);
                if (next_sample_idx >= 0 && next_sample_idx < all_samples.size())
                    prev_set_sample += all_samples[next_sample_idx] / (double)prev.tracking_info_per_rep.size();
            }
            //allele
			if(curr_set_sample == 0) curr_set_sample = min_non_zero_prev_sample;
			if(prev_set_sample == 0) prev_set_sample = min_non_zero_prev_sample;
...
//    }
    ...
    for (size_t i = 0; i < prev_abundance.abundances().size(); ++i)
    {
        FPKMPerReplicateTable ab_i_by_rep;
		if(!prev_parent)
		{
			ab_i_by_rep = prev_abundance.abundances()[i]->paternal_FPKM_by_replicate();
		}
		else
			{
			ab_i_by_rep = prev_abundance.abundances()[i]->maternal_FPKM_by_replicate();
		}
    ...
    for (size_t i = 0; i < curr_abundance.abundances().size(); ++i)
    {
		...
		if(!curr_parent)
		{
			ab_i_by_rep	= curr_abundance.abundances()[i]->paternal_FPKM_by_replicate();
		}
		else
		{
			ab_i_by_rep	= curr_abundance.abundances()[i]->maternal_FPKM_by_replicate();
		}
    ...
    boost::random::mt19937 rng;
	long prev_size,curr_size;
	if(!prev_parent)
		prev_size = prev_abundance.paternal_member_fpkm_samples().size();
	else
		prev_size = prev_abundance.maternal_member_fpkm_samples().size();
	if(!curr_parent)
		curr_size = curr_abundance.paternal_member_fpkm_samples().size();
	else
		curr_size = curr_abundance.maternal_member_fpkm_samples().size();
    boost::random::uniform_int_distribution<> prev_sampler(0, prev_size-1);
    boost::random::uniform_int_distribution<> curr_sampler(0, curr_size-1);
    ...
    for (size_t i = 0; i < num_null_js_samples; ++i)
    {
        ...
        {
            int next_sample_idx = prev_sampler(rng);
            if (next_sample_idx >= 0 && next_sample_idx < prev_size){
				if(!prev_parent)
					curr_set_sample += prev_abundance.paternal_member_fpkm_samples()[next_sample_idx] / (double)curr_abundance.rg_props().size();
				else
					curr_set_sample += prev_abundance.maternal_member_fpkm_samples()[next_sample_idx] / (double)curr_abundance.rg_props().size();
			}
		}
        double total_curr_set_sample_fpkm = curr_set_sample.sum();
        if (total_curr_set_sample_fpkm > 0.0)
        ...
        for (size_t k = 0; k < prev_abundance.rg_props().size(); ++k)
        {
            ...
            if (next_sample_idx >= 0 && next_sample_idx < prev_size){
				if(!prev_parent)
					prev_set_sample += prev_abundance.paternal_member_fpkm_samples()[next_sample_idx] / (double)prev_abundance.rg_props().size();
				else
					prev_set_sample += prev_abundance.maternal_member_fpkm_samples()[next_sample_idx] / (double)prev_abundance.rg_props().size();
			}
		}
        double total_prev_set_sample_fpkm = prev_set_sample.sum();
        if (total_prev_set_sample_fpkm > 0.0)
    ...
    for (size_t i = 0; i < num_null_js_samples; ++i)
    {
        ...
        {
            int next_sample_idx = curr_sampler(rng);
            if (next_sample_idx >= 0 && next_sample_idx < curr_size){
				if(!curr_parent)
					curr_set_sample += curr_abundance.paternal_member_fpkm_samples()[next_sample_idx] / (double)curr_abundance.rg_props().size();
				else
					curr_set_sample += curr_abundance.maternal_member_fpkm_samples()[next_sample_idx] / (double)curr_abundance.rg_props().size();
			}
		}
        double total_curr_set_sample_fpkm = curr_set_sample.sum();
        if (total_curr_set_sample_fpkm > 0.0)
        ...
        for (size_t k = 0; k < prev_abundance.rg_props().size(); ++k)
        {
            ...
            if (next_sample_idx >= 0 && next_sample_idx < curr_size){
				if(!curr_parent)
					prev_set_sample += curr_abundance.paternal_member_fpkm_samples()[next_sample_idx] / (double)prev_abundance.rg_props().size();
				else
					prev_set_sample += curr_abundance.maternal_member_fpkm_samples()[next_sample_idx] / (double)prev_abundance.rg_props().size();
			}
		}
        double total_prev_set_sample_fpkm = prev_set_sample.sum();
        if (total_prev_set_sample_fpkm > 0.0)
							  ...
							  bool doTest=true)
{
    ...
    if (prev_abundance.abundances().size() == 1 ||
        prev_num_fragments == 0 ||
        curr_num_fragments == 0)
    {
        test.p_value = 1;
        test.value_1 = 0;
        test.value_2 = 0;
                        ...
                        shared_ptr<AlleleTestLauncher> launcher)
{
    ...
    std::vector<boost::shared_ptr<BundleFactory> > factories = sample_factory.factories();
    vector<shared_ptr<PrecomputedAlleleExpressionBundleFactory> > hit_factories;
	
    for (size_t i = 0; i < factories.size(); ++i)
    {
        shared_ptr<BundleFactory> pFac = factories[i];
        shared_ptr<PrecomputedAlleleExpressionBundleFactory> pBundleFac = dynamic_pointer_cast<PrecomputedAlleleExpressionBundleFactory> (pFac);
        if (pBundleFac)
        {
            // If we get here, this factory refers to a pre computed expression object.
    ...
    if (hit_factories.size() == factories.size())
    {
        merge_precomputed_allele_expression_worker(boost::cref(locus_tag),
                                            hit_factories,
                                            boost::ref(*abundance),
                                            bundle,
                                            perform_cds_analysis,
...
//allele
void clear_samples_from_tracking_table(shared_ptr<SampleAlleleAbundances> sample, Tracking& tracking)
{
    for (size_t k = 0; k < sample->transcripts.abundances().size(); ++k)
    {
        const Abundance& abundance = *(sample->transcripts.abundances()[k]);
    ...
    if(samples.size() > 1){
		for (size_t contrast_idx = 0; contrast_idx < contrasts.size(); ++contrast_idx)
		{
			...
			for (size_t k = 0; k < samples[i]->gene_primary_transcripts.size(); ++k)
			{
				...
				bool paternal_paternal_enough_reads = (curr_abundance.paternal_FPKM_by_replicate().size() >= min_reps_for_js_test && 
													   prev_abundance.paternal_FPKM_by_replicate().size() >= min_reps_for_js_test);
					
				bool paternal_maternal_enough_reads = (curr_abundance.paternal_FPKM_by_replicate().size() >= min_reps_for_js_test && 
													   prev_abundance.maternal_FPKM_by_replicate().size() >= min_reps_for_js_test);
				
				bool maternal_paternal_enough_reads = (curr_abundance.maternal_FPKM_by_replicate().size() >= min_reps_for_js_test && 
													   prev_abundance.paternal_FPKM_by_replicate().size() >= min_reps_for_js_test);
				
				bool maternal_maternal_enough_reads = (curr_abundance.maternal_FPKM_by_replicate().size() >= min_reps_for_js_test && 
													   prev_abundance.maternal_FPKM_by_replicate().size() >= min_reps_for_js_test);
...
//      
		...
		for (size_t k = 0; k < samples[0]->transcripts.abundances().size(); ++k)
		{
			...
			SampleDifference paternalMaternalTest;
			paternalMaternalTest = get_de_tests(desc,
...
#endif
			pair<SampleDiffs::iterator, bool> paternalMaternalInserted; 
			paternalMaternalInserted = tests.parental_isoform_de_tests[0][0].insert(make_pair(desc,
																					 paternalMaternalTest)); 
		...
		for (size_t k = 0; k < samples[0]->cds.size(); ++k)
		{
			...
			pair<SampleDiffs::iterator, bool> paternalMaternalInserted; 
			paternalMaternalInserted = tests.parental_cds_de_tests[0][0].insert(make_pair(desc,
																				 paternalMaternalTest)); 
			shared_ptr<SampleDifferenceMetaData> meta_data = get_metadata(desc);
...
#endif
			pair<SampleDiffs::iterator, bool> paternalMaternalInserted; 
			paternalMaternalInserted = tests.parental_tss_group_de_tests[0][0].insert(make_pair(desc,
																				 paternalMaternalTest)); 
...
#endif
			pair<SampleDiffs::iterator, bool> paternalMaternalInserted; 
			paternalMaternalInserted = tests.parental_gene_de_tests[0][0].insert(make_pair(desc,
																				  paternalMaternalTest)); 
		...
		if (!no_js_tests)
		{
			...
			for (size_t k = 0; k < samples[0]->gene_primary_transcripts.size(); ++k)
			{
				...
				pair<SampleDiffs::iterator, bool> paternal_maternal_inserted;
				paternal_maternal_inserted = tests.parental_diff_promoter_tests[0][0].insert(make_pair(desc,paternalMaternalTest)); 
				paternal_maternal_inserted.first->second = paternalMaternalTest;
			...
			for (size_t k = 0; k < samples[0]->gene_cds.size(); ++k)
			{
				...
				pair<SampleDiffs::iterator, bool> paternal_maternal_inserted;
				paternal_maternal_inserted = tests.parental_diff_cds_tests[0][0].insert(make_pair(desc,paternalMaternalTest)); 
				paternal_maternal_inserted.first->second = paternalMaternalTest;
